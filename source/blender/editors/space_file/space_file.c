...
static void file_keymap(struct wmKeyConfig *keyconf)
{
	...
	WM_keymap_add_item(keymap, "FILE_OT_select", LEFTMOUSE, KM_RELEASE, 0, 0);
	kmi = WM_keymap_add_item(keymap, "FILE_OT_select", LEFTMOUSE, KM_RELEASE, KM_SHIFT, 0);
	RNA_boolean_set(kmi->ptr, "extend", true);
	kmi = WM_keymap_add_item(keymap, "FILE_OT_select", LEFTMOUSE, KM_RELEASE, KM_CTRL | KM_SHIFT, 0);
	RNA_boolean_set(kmi->ptr, "extend", true);
	RNA_boolean_set(kmi->ptr, "fill", true);

	/* right mouse selects without opening */
	kmi = WM_keymap_add_item(keymap, "FILE_OT_select", RIGHTMOUSE, KM_RELEASE, 0, 0);
	RNA_boolean_set(kmi->ptr, "open", false);
	kmi = WM_keymap_add_item(keymap, "FILE_OT_select", RIGHTMOUSE, KM_RELEASE, KM_SHIFT, 0);
	RNA_boolean_set(kmi->ptr, "extend", true);
	RNA_boolean_set(kmi->ptr, "open", false);
	kmi = WM_keymap_add_item(keymap, "FILE_OT_select", RIGHTMOUSE, KM_RELEASE, KM_ALT, 0);
	RNA_boolean_set(kmi->ptr, "extend", true);
	RNA_boolean_set(kmi->ptr, "fill", true);
	RNA_boolean_set(kmi->ptr, "open", false);

	/* front and back mouse folder navigation */
	WM_keymap_add_item(keymap, "FILE_OT_previous", BUTTON4MOUSE, KM_RELEASE, 0, 0);
	WM_keymap_add_item(keymap, "FILE_OT_next", BUTTON5MOUSE, KM_RELEASE, 0, 0);
